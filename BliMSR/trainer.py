...
def parse_args():
  p = argparse.ArgumentParser(description='Train the BliMSR base super-resolution model.')
  p.add_argument('--data_path', default='data/train_data.pt', help='(hr, lr) tensor pairs, see prepare_data.py')
...
def main():
  ...
  log_name = os.path.join(args.checkpoints_dir, 'loss_log.txt')

  for epoch in range(args.start_epoch, args.epochs + 1):
    e_loss_G, e_loss_D = [], []

    for hr_img, lr_img in tqdm(train_dl):
      hr_img, lr_img = hr_img.to(device), lr_img.to(device)
      ...
      with torch.cuda.amp.autocast():
        ...
        perc_loss = perceptual_loss(pred_hr, hr_img, feature_extractor, loss_function)
    ...
    if epoch % args.save_every == 0 or epoch == args.epochs:
      torch.save(gen.state_dict(), os.path.join(args.checkpoints_dir, f'gen_{epoch}.pth'))
      torch.save(disc.state_dict(), os.path.join(args.checkpoints_dir, f'disc_{epoch}.pth'))
